...
class YOLOV5ToCOCO(object):
    ...
    def __call__(self, mode_list: list):
        ...
        for mode in mode_list:
            ...
            self.label_dir = self.raw_data_dir / 'labels' / f'{mode}'
    ...
    def convert(self, img_list, save_img_dir, mode):
        ...
        for img_id, img_path in enumerate(tqdm(img_list, desc=mode), 1):
            img_path = os.path.join(self.raw_data_dir, img_path)
            image_dict = self.get_image_info(img_path, img_id, save_img_dir)
            # print(img_path)
            if image_dict:
                images.append(image_dict)
                label_path = self.label_dir / f'{Path(img_path).stem}.txt'
    ...
    def get_image_info(self, img_path, img_id, save_img_dir):
        ...
        if not self.verify_exists(img_path):
            return None
...
if __name__ == "__main__":
    ...
    parser.add_argument('--dir_path', type=str, default='fast-data/guest/',
